  ...
  public  class CollectionPager : System.Web.UI.Control, IPostBackEventHandler
	{
        ...
        protected string _ResultsFormat = "Displaying results {0} - {1} ({2} Results)";
		...
		protected string _LabelStyle = "FONT-WEIGHT: bold;MARGIN-RIGHT:7px";
		...
		protected string _BackText	= "Previous";
        ...
        [Bindable(false), Category("Previous/Next")] 
		...
		protected override void OnPreRender(EventArgs e)
		{
            ...
            if (_CurrentPage > _PagedDataSet.PageCount)
                _CurrentPage = 1;
		...
		protected virtual void BuildControl()
		{
            ...
            //Already remove,    if want to add, use  _LabelText = "Page:";
            ...
            // If Back/Next == Left or Split   Value = Pervious
            if ((BackNextLocation == BackNextLocationType.Right
                || BackNextLocation == BackNextLocationType.Split) && CurrentPage != 1)
            {
                if (_BackNextStyle.Length > 0)
                {
                    Html.AppendFormat("<span style='{0}'>", _BackNextStyle);
			...
			if(ShowPageNumbers)
			{
				if(ShowLabel)
				{
					Html.Append("<span style='margin-left:5px'></span>");
				}
				else if(IsSpaceNeeded)
				{
                    Html.AppendFormat("<span style='margin-left:{0}px'></span>", SectionPadding);
				}
			...
			// If Back/Next == Left or Split
			if((BackNextLocation == BackNextLocationType.Right
                || BackNextLocation == BackNextLocationType.Split) && CurrentPage != PageCount)
			{
				if(IsSpaceNeeded)
				{
					Html.AppendFormat("<span style='margin-left:{0}px'></span>", SectionPadding);
				...
				else
				{
					...
					else
					{
						...
						if(PagingMode==PagingModeType.PostBack)
						{
							if(PageNumbersDisplay==PageNumbersDisplayType.Numbers)
							{
                                Pages.AppendFormat("<a id=\"{0}\" class=\"CollectionPagerBorder\" href=\"javascript:{1}\"{2}>{3}</a>"
									, this.UniqueID
							...
							{
                                Pages.AppendFormat("[<a id=\"{0}\" class=\"CollectionPagerBorder\" href=\"'javascript:{1}\"{2}>{3}-{4}</a>]"
									, this.UniqueID
                                    , Page.ClientScript.GetPostBackEventReference(this, i.ToString())
									, PageNumberStyleTag
						...
						else
						{
							if(PageNumbersDisplay==PageNumbersDisplayType.Numbers)
							{
                                Pages.AppendFormat("<a href='{0}'{1} class=\"CollectionPagerBorder\">{2}</a>"
							...
							{
                                Pages.AppendFormat("[<a href='{0}'{1} class=\"CollectionPagerBorder\">{2} {3}</a>]"
									, UpdateQueryStringItem(Context.Request, QueryStringKey, i.ToString())
									, PageNumberStyleTag
									, RecordStart
									, RecordEnd);
				...
				if(PagingMode==PagingModeType.PostBack)
				{
					if(ShowFirstLast)
					{
						LinkNav.AppendFormat("<a id=\"{0}\" href=\"javascript:{1}\">{2}</a>&nbsp;"
							, this.UniqueID
                            , Page.ClientScript.GetPostBackEventReference(this, Convert.ToString(1))
							, FirstText);
					}

					LinkNav.AppendFormat("<a id=\"{0}\" href=\"javascript:{1}\">{2}</a>"
						, this.UniqueID
                        , Page.ClientScript.GetPostBackEventReference(this, Convert.ToString(currentPage - 1))
						, BackText);
				}
				else
				{
			...
			else
			{
				...
				if(PagingMode==PagingModeType.PostBack)
				{
					...
					if(ShowFirstLast)
					{
						LinkNav.AppendFormat("&nbsp;<a id=\"{0}\" href=\"javascript:{1}\">{2}</a>"
							, this.UniqueID
                            , Page.ClientScript.GetPostBackEventReference(this, Convert.ToString(pageCount))
							, LastText);
					}
				}
				else
		...
		static public string UpdateQueryStringItem(System.Web.HttpRequest httpRequest, string queryStringKey, string newQueryStringValue)
		{
			...
			else if(newQueryStringValue.Trim()!="")
			{
                ...
                if (httpRequest.QueryString.Count == 0 ||httpRequest.RawUrl.IndexOf('?')<0)
				{
