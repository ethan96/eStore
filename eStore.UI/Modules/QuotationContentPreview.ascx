<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="QuotationContentPreview.ascx.cs"
    Inherits="eStore.UI.Modules.QuotationContentPreview" %>
...
<asp:Panel ID="pComment" runat="server">
    <div class="DarkBlueHeader">
        <%=eStore.Presentation.eStoreLocalization.Tanslation(eStore.POCOS.Store.TranslationKey.Quotation_Comment)%>
    </div>
    <p>
...
<div class="hiddenitem eStore_orderStep_subTotal" labfor="cartTotal">
    ...
    <%} %>
    <%if (eStore.Presentation.eStoreContext.Current.Quotation.DutyAndTax.GetValueOrDefault() > 0)
      {%>
    <div>
        <span>
    ...
    <%} %>
    <%if (eStore.Presentation.eStoreContext.Current.Quotation.TotalDiscount.GetValueOrDefault() > 0)
      {%>
    <div>
        <span class="colorRed">
            <%=eStore.Presentation.eStoreLocalization.Tanslation(eStore.POCOS.Store.TranslationKey.Cart_Discount)%></span>
        <span>
...
<div class="eStore_addReceiverBlock" id="dContactDetailsBill">
    ...
    <div class="eStore_order_btnBlock">
        <asp:Button ID="btSaveBillto" runat="server" OnClientClick="return CheckValidate('#cartContactContext')" Text="Save" CssClass="eStore_btn" OnClick="btnSaveUserContact_Click" />
