...
(function($, window, document, undefined) {
  ...
  var Slider = {
    ...
    addNavigation: function(navClass) {
      var self = this,
        dynamicTabsElm = '<' + self.options.navElementTag + ' class="ls-nav' + (self.options.dynamicTabsThumbnail?" ls-nav-thumbnail":self.options.dynamicTabsNumber?" ls-nav-num":"")+'"><ul id="' +
      ...
      $.each(
        ...
        function(n) {
          $((self.$sliderWrap)).find('.ls-nav ul').append('<li class="tab' +
            ...
            (n + 1) + '">' +
            (self.options.dynamicTabsThumbnail?self.getThumbnails(this):self.options.dynamicTabsNumber?(n+1):self.getNavInsides(this)) + '</a></li>');
          if (!self.options.includeTitle) $(this).remove();
        }
      );
    },
    getThumbnails: function(input) {
 return   '<img src="' + $(input).parent().find("img").attr("thumbnail") +'"/>' 
    ...
    registerNav: function() {
      var self = this;
      (self.$sliderWrap).find('[class^=ls-nav] li').bind('click', function() {
    ...
    registerArrows: function() {
      var self = this;
      $((self.$sliderWrap).find('[class^=ls-nav-]')).bind('click', function() {
    ...
    registerCrossLinks: function() {
      ...
      (self.crosslinks).bind('click', function(e) {
    ...
    events: function() {
      ...
      (self.$sliderWrap).find('*').bind('click', function() {
        if (self.options.forceAutoSlide)
          self.startAutoSlide(true);
  ...
  $.fn.liquidSlider.options = {
    ...
    dynamicTabs: true,
    dynamicTabsNumber: true,
    dynamicTabsThumbnail:false,
...
function adjustTabWdith() {
    var tabs = $(".ls-nav ul");

    if ($(tabs).data("init") == undefined) {
        var width = $(tabs).parent().outerWidth();
        $(tabs).css("width", width + "px");
        var tabstotalwidth = 0;
        var padding = 0;
        var tabscount = 0;
        $(tabs).find("li a").each(function (j, n) {
            tabscount++;
            tabstotalwidth += $(n).outerWidth();
        });
        padding = (width - tabstotalwidth - tabscount * 21) / tabscount;
        $(tabs).find("li a").each(function (j, n) {
